...
```{sql connection=con}
-- Your code goes here
SELECT EXTRACT(YEAR FROM start) as billing_year, 
       sum(payer_coverage) as total_reimbursement
FROM ENCOUNTERS
...
-- Your code goes here
SELECT EXTRACT(YEAR FROM start) as billing_year, 
       sum(payer_coverage)/count(DISTINCT patient) as total_reimbursement
FROM ENCOUNTERS
INNER JOIN payers on encounters.payer = payers.id
WHERE payers.name = 'Medicare'
GROUP BY billing_year
ORDER BY billing_year DESC
```
